  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (childRes.success && childRes.child) {
        ...
        const childId = childRes.child.student_id;
        
        // Fetch timetable using student ID
        const res = await apiGet(`/core/timetable/student/${childId}`);
        if (res.success) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#C4B5FD]/70 font-light">
              Viewing daily schedules and class placements for <span className="font-bold text-white">{childName || 'your child'}</span>
            </p>
          </div>
        </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 text-[10px] text-[#C4B5FD]/70 mt-1 font-light flex-wrap">
                          ...
                          <span>•</span>
                          <span className="flex items-center gap-1"><User className="w-3.5 h-3.5" /> Instructor: {item.staff?.users?.full_name || item.staff?.users?.name || 'Prof. Vyas'}</span>
                        </div>
                      </div>
                    </div>
