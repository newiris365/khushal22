  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.declaration) {
        ...
        setHra(d.hra_claimed || 0);
        setSec80c(d.section_80c || 0);
        setSec80d(d.section_80d || 0);
        setSec80g(d.section_80g || 0);
        setHomeLoan(d.home_loan_interest || 0);
      }
    } catch (err) {
      console.error('Failed to load TDS declaration:', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(data.error || 'Failed to save TDS declaration.');
      }
    } catch (err) {
      alert('Network error submitting TDS declarations.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="max-w-7xl mx-auto py-2 w-full flex flex-col gap-6">
      ...
      {loading ? (
        <div className="text-center py-20">
          ...
          <p className="text-xs text-[#C4B5FD]">Loading tax declarations...</p>
        </div>
      ) : (
